...
# driver.get('https://www.malaysia.gov.my/portal/agencydir')
...
#
#
# with open('Government9.csv', 'w') as f:
...
#
# # filter unwanted texts
# open('Governmentfinal9.csv', 'w').writelines(line for line in open('Government9.csv'))
#                                             # if not 'Petronas' or 'location' or 'PETRONAS' in line)
#
# if (os.path.isfile('Government9.csv') and os.path.isfile('Governmentfinal9.csv')):
#     os.remove('Government9.csv')
#     print("file deleted")
# else:
...
# with open('Government.csv', 'r') as read_obj:
...
with open('Governmentfinalb.csv', 'w') as f:
...
df = pd.read_csv("Governmentfinala.csv")
# checking the number of empty rows in th csv file
print(df.isnull().sum())
# Droping the empty rows
modifiedDF = df.dropna()
# Saving it to the csv file
modifiedDF.to_csv('Governmentfinalb.csv', index=False)
...
cur.execute('DROP TABLE IF EXISTS Government')
cur.execute('''
...
if len(fname) < 1 : fname = "Governmentfinalb.csv"
...
with open(fname) as csv_file:
    ...
    for row in csv_reader:
        ...
        # lat = lat.strip("\"")
        # lng = lng.strip("\"")
        cur.execute('''INSERT INTO Government (locations,lat,lng)
