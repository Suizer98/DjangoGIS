...
driver.get('https://www.shell.com.my/motorists/shell-recharge/shell-recharge-hpc.html#hpc-locations')
...
elements = driver.find_elements(By.CLASS_NAME, "interactive-image__hotspot-text")
...
#
# # filter unwanted texts
# open('Shellfinal.csv', 'w').writelines(line for line in open('Shell.csv') if not 'location link' in line)
#
# if (os.path.isfile('Shell.csv') and os.path.isfile('Shellfinal.csv')):
#     os.remove('Shell.csv')
#     print("file deleted")
# else:
...
data = pd.read_csv("Shellfinal.csv")
...
with open('Shellfinal2.csv', 'w') as f:
...
df = pd.read_csv("Shellfinal2.csv")
# checking the number of empty rows in th csv file
print(df.isnull().sum())
# Droping the empty rows
modifiedDF = df.dropna()
# Saving it to the csv file
modifiedDF.to_csv('Shellfinal2.csv', index=False)
...
cur.execute('DROP TABLE IF EXISTS Shell')
cur.execute('''
		CREATE TABLE "Shell" (
			"locations" TEXT,
			"lat" TEXT,
...
if len(fname) < 1 : fname = "Shellfinal2.csv"
...
with open(fname) as csv_file:
    ...
    for row in csv_reader:
        ...
        # lat = lat.strip("\"")
        # lng = lng.strip("\"")
        cur.execute('''INSERT INTO Shell (locations,lat,lng)
